...
let gen ((w, h) as dim) : state =
  let columns =
    1 -- w |> List.map (fun _ -> if Random.float 1. < 0.1 then gen_line h else gen_wait h)
  in
  dim, columns
;;
  ...
  Matrix.run
    ...
    ~on_input:(fun app event ->
      ...
      | Input.Key { key = Input.Key.Char uchar; modifier; _ } ->
        ...
        if code = 3 || (modifier.ctrl && (code = Char.code 'c' || code = Char.code 'C'))
